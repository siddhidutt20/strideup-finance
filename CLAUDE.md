...
7. **A closed month is not rewritten.** A late document dated inside it posts
   to the open month as an adjustment, with a note saying so.
...
- **A bank feed.** The largest known gap: the headline figure is
  ...
  A forgotten receipt is invisible. This is Phase 1 of the next version.
- **Multi-user, alerts, email capture, live GoHighLevel.** All planned, none
  built.
