...
// ── Revenue ──────────────────────────────────────────────────
export function RevenueView({ st, trend, money, period }) {
  const paid = st.revenue.byCategory.reduce((a, r) => a + r.amount, 0);
  return (
    <>
      <section className="fin-kpis">
        <Kpi label="Revenue this month" value={money.round(paid)} tone="in" emphasis />
        <Kpi label="Outstanding" value={money.round(st.revenue.receivables.total)}
             hint={st.revenue.receivables.overdue > 0
               ? `${money.round(st.revenue.receivables.overdue)} overdue` : "none overdue"}
             tone={st.revenue.receivables.overdue > 0 ? "warn" : undefined} />
        <Kpi label="Customers" value={String(st.revenue.byCustomer.length)}
             hint="paid this month" />
      </section>
      <Panel title="Revenue by month" sub="Trailing view">
        <TrendChart series={trend} money={money} current={period} only="revenue" />
      </Panel>
      <div className="fin-grid">
        <Panel title="By category" sub={monthLabel(period)}>
          <CategoryBars rows={st.revenue.byCategory} money={money}
                        empty="No revenue recorded this month." />
        </Panel>
        <Panel title="By customer" sub={monthLabel(period)}>
          <Ranked rows={st.revenue.byCustomer} money={money} tone="in"
                  empty="No customer payments this month." />
        </Panel>
      </div>
      <Panel title="Outstanding payments" sub="Everything not yet settled">
        <Receivables ar={st.revenue.receivables} money={money} />
      </Panel>
    </>
  );
}
...
// ── Expenses ─────────────────────────────────────────────────
export function ExpensesView({ st, trend, money, period }) {
  const total = st.expenses.byCategory.reduce((a, r) => a + r.amount, 0);
  const biggest = st.expenses.byCategory[0];
  return (
    <>
      <section className="fin-kpis">
        <Kpi label="Spent this month" value={money.round(total)} tone="out" emphasis />
        <Kpi label="Largest category" value={biggest ? money.round(biggest.amount) : "—"}
             hint={biggest?.name || "nothing recorded"} />
        <Kpi label="Suppliers" value={String(st.expenses.byVendor.length)} hint="paid this month" />
      </section>
      <Panel title="Expenses by month" sub="Trailing view">
        <TrendChart series={trend} money={money} current={period} only="expenses" />
      </Panel>
      <div className="fin-grid">
        <Panel title="By category" sub={monthLabel(period)}>
          <CategoryBars rows={st.expenses.byCategory} money={money}
                        empty="No expenses recorded this month." />
        </Panel>
        <Panel title="By supplier" sub={monthLabel(period)}>
          <Ranked rows={st.expenses.byVendor} money={money} tone="out"
...
// ── Cash flow ────────────────────────────────────────────────
// Direct method. Capital sits on its own line because it is not trading
...
export function CashflowView({ st, money, period }) {
  ...
  const row = (label, sub, amount, tone) => (
    <div className="cf-row" key={label}>
      ...
      <span className={tone}>
        {tone === "cf-in" ? "+" : tone === "cf-out" ? "−" : ""}
        {money.exact(Math.abs(amount))}
      </span>
    </div>
  );
  return (
    <>
      <Panel narrow sub="Opening position, what moved, and where it left you">
      ...
      <Panel narrow title="Capital" sub="Equity, loans and draws to date">
        <CapitalList capital={st.capital} money={money} />
      </Panel>
    </>
  );
}
